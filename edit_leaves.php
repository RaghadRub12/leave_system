...
$query = "SELECT * FROM leaves WHERE id = '$clean_id'"; // safe
...
if ($_POST) {
    ...
    if ($employee_name && $leave_type) {
        ...
        $stmt->bind_param('ssssssi',  $employee_name, $leave_type,  $applied_date,  $hod_status, $registered_status,$registered_date,$id);
...
?>
    <div class="container">
        <h1> Employee</h1>
        <hr>
        <h2 class="mb-4">Edit Leaves</h2>

        <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="post">
            <div class="row mb-3">
                <label for="name" class="col-sm-2 col-form-label">Name</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" id="name" value="<?= htmlspecialchars($data['employee_name']) ?>">
                </div>
            </div>
            <div class="row mb-3">
                <label for="applied_date" class="col-sm-2 col-form-label">Applied Date</label>
                <div class="col-sm-10">
                    <input type="date" class="form-control" id="applied_date" name="applied_date" value="<?= htmlspecialchars($data['applied_date']) ?>"/>
                </div>
            </div>
            <div class="row mb-3">
                ...
                <select class="form-select" id="hod_status" name="hod_status">
                        ...
                        <option value="<?= $key ?>" <?= $key == $data['hod_status'] ? 'selected' : '' ?> ><?= $value ?></option>
                        <?php endforeach ?>
                    </select>
                </div>
            </div>
            <div class="row mb-3">
                ...
                <select class="form-select" id="registered_status" name="registered_status">
                        <option></option>
                        <?php foreach ($status as $key => $value) : ?>
                        <option value="<?= $key ?>" <?= $key == $data['registered_status'] ? 'selected' : '' ?> ><?= $value ?></option>
                        <?php endforeach ?>
                    </select>
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <input type="date" class="form-control" id="registered_date" name="registered_date" value="<?= htmlspecialchars($data['registered_date']) ?>" />
                </div>
            </div>
            <div class="row mb-3">
